...
export default function RhymeClusterPanel({ cluster, clusters = [] }) {
  ...
  const activeSavedCluster = savedClusters.find((entry) => entry.id === activeCluster?.id) ?? null;
  ...
  useEffect(() => {
    setManageTitle(activeSavedCluster?.title ?? "");
    setManageStatus("");
  }, [activeSavedCluster?.id, activeSavedCluster?.title]);
  ...
  const handleRenameSavedCluster = () => {
    if (!activeSavedCluster) return;
    const title = manageTitle.trim();
    if (!title) return;
    const next = savedClusters.map((entry) =>
      entry.id === activeSavedCluster.id ? { ...entry, title } : entry,
    );
    setSavedClusters(next);
    writeSavedClusters(next);
    setManageStatus("Renamed");
  };

  const handleDeleteSavedCluster = () => {
    if (!activeSavedCluster) return;
    const next = savedClusters.filter((entry) => entry.id !== activeSavedCluster.id);
    setSavedClusters(next);
    writeSavedClusters(next);
    const fallbackId = next[0]?.id ?? sourceClusters[0]?.id ?? customCluster?.id ?? "";
    setActiveClusterId(fallbackId);
        ...
        {activeSavedCluster && (
          <div className="mt-4 rounded-2xl border border-white/10 bg-black/40 p-4">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <button
                type="button"
                onClick={handleRenameSavedCluster}
                disabled={!manageTitle.trim()}
                className="rounded-full border border-crystal/60 px-4 py-1.5 text-xs font-semibold text-crystal transition hover:bg-crystal hover:text-ink disabled:cursor-not-allowed disabled:border-white/20 disabled:text-paperWhite/40"
              >
                Rename
              </button>
              <button
                type="button"
                onClick={handleDeleteSavedCluster}
                className="rounded-full border border-rose-400/60 px-4 py-1.5 text-xs font-semibold text-rose-300 transition hover:bg-rose-400/20"
              >
                Delete
              </button>
            </div>
            <p className="mt-2 text-xs text-paperWhite/45">
              {manageStatus
                ? `Saved preset status: ${manageStatus}.`
                : "Rename or delete this saved preset."}
            </p>
          </div>
        )}
      </div>
