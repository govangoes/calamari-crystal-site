...
function shouldConvert(fp) {
  ...
  if (base.startsWith("favicon") || base.startsWith("icon-") || base.includes("apple"))
    return false;
  return true;
}
function bytes(n) {
...
async function main() {
  ...
  for (const fp of walk(root)) {
    ...
    try {
      ...
      console.log(
        `[img:opt] ${path.relative(process.cwd(), fp)} -> ${path.basename(outWebp)} (${bytes(buf.length)} → ${bytes(webp.length)})`,
      );
      converted++;
    } catch (e) {
